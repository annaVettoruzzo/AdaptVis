"""
Compute attention rollout for POPE dataset using base LLaVA 1.5 model.

This script:
1. Loads POPE dataset
...
def main():
    parser = argparse.ArgumentParser(description='Compute attention rollout for POPE')
    parser.add_argument("--groups-filter", default=None, type=str,
            help="Comma-separated list of categories to include (e.g., 'adversarial,popular')")
    parser.add_argument("--n-samples", default=10, type=int,
            help="Number of samples per category (default: 10 for testing)")
    parser.add_argument("--seed", default=42, type=int,
            help="Random seed")
    parser.add_argument("--output-dir", default="./output/pope_base",
    ...
    print("=" * 60)
    print("POPE Attention Rollout - Base LLaVA 1.5")
    print("=" * 60)

    # Parse POPE arguments
    dataset_kwargs = {}
    ...
    dataset = get_dataset("POPE", image_preprocess=image_preprocess, **dataset_kwargs)
    ...
    for idx, batch in enumerate(tqdm(dataloader, desc="Generating")):
        ...
        pope_question = batch["caption_options"][0][0]
        ...
        if (idx + 1) % 10 == 0:
            ...
            print(f"   Latest generation: {generated_text}")
    ...
    print(f"     python visualize_all_pope.py")
    print(f"\n  Option 3 - Visualize specific range:")
    print(f"     python visualize_all_pope.py --start 0 --max-samples 10")
    print("=" * 60)
